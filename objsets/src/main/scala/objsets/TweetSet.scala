...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    ...
    else filterAcc(t => true, this) //way better with accumulator...
  }
  ...
  def mostRetweeted: Tweet = {
    def _mostRet(me: Tweet, him: Tweet): Tweet =
      if (me.retweets > him.retweets) me else him

    if (right.isEmpty && left.isEmpty) elem
    else if (right.isEmpty) _mostRet(left.mostRetweeted, elem)
    else if (left.isEmpty) _mostRet(right.mostRetweeted, elem)
    else _mostRet(elem, _mostRet(left.mostRetweeted, right.mostRetweeted))
  }
...
object GoogleVsApple {
  ...
  lazy val appleTweets: TweetSet = {
    TweetReader.allTweets.filter(t => apple.exists(keyword => t.text.contains(keyword)))
  }
